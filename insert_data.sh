...
cat games.csv | while IFS="," read YEAR ROUND WINNER OPPONENT WINNER_GOALS OPPONENT_GOALS
   ...
   then
      ...
      if [[ -z $TEAM_ID ]]
      then
      # insert name from winner and opponent
        INSERT_NEW_NAME=$($PSQL "INSERT INTO teams(name) VALUES('$WINNER')")
        INSERT_NEW_NAME=$($PSQL "INSERT INTO teams(name) VALUES('$OPPONENT')")
      ...
      GAME_ID=$($PSQL "SELECT game_id FROM games WHERE winner_goals == opponent_goals") 
        ...
        INSERT_NEW_INFOS=$($PSQL "INSERT INTO games(year, round, winner_id, opponent_id, winner_goals, opponent_goals) 
